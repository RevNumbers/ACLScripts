...
function source_match
{
    ...
    SHOST=$(echo $line | awk '{print $3}')
        ...
        SMATCHSUB24="$(grep -w $CLASS24 inbound_OBJs_src/* | head -1)"
        SMATCHSUB24A=($(grep -w $CLASS24 inbound_OBJs_src/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        SMATCHSUB23="$(grep -w $CLASS23 inbound_OBJs_src/* | head -1)"
        SMATCHSUB23A=($(grep -w $CLASS23 inbound_OBJs_src/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        SMATCHSUB16="$(grep -w $CLASS16 inbound_OBJs_src/* | head -1)"
        SMATCHSUB16A=($(grep -w $CLASS16 inbound_OBJs_src/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        SMATCHSUB12="$(grep -w $CLASS12 inbound_OBJs_src/* | head -1)"
        SMATCHSUB12A=($(grep -w $CLASS12 inbound_OBJs_src/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        SMATCHSUB8="$(grep -w $CLASS8 inbound_OBJs_src/* | head -1)"
        SMATCHSUB8A=($(grep -w $CLASS8 inbound_OBJs_src/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        ...
        if test -n ${SMATCHSUB23A[0]}
          then
            ...
            for file in ${SMATCHSUB23A[@]}
              do
                if [[ $(grep -w $CLASS23 inbound_OBJs_src/$file | awk '{print $3}') =~ "255.255.254.0" ]]
                  then
                    SMATCH23ARR+=($file)
                fi
            done
        ...
        if test -n ${SMATCHSUB16A[0]}
          then
            ...
            for file in ${SMATCHSUB16A[@]}
              do
                if [[ $(grep -w $CLASS16 inbound_OBJs_src/$file | awk '{print $3}') =~ "255.255.0.0" ]]
                  then
                    SMATCH16ARR+=($file)
                fi
            done
        ...
        if test -n ${SMATCHSUB12A[0]}
          then
            ...
            for file in ${SMATCHSUB12A[@]}
              do
                if [[ $(grep -w $CLASS12 inbound_OBJs_src/$file | awk '{print $3}') =~ "255.240.0.0" ]]
                  then
                    SMATCH12ARR+=($file)
                fi
            done
        ...
        if test -n ${SMATCHSUB8A[0]}
          then
            ...
            for file in ${SMATCHSUB8A[@]}
              do
                if [[ $(grep -w $CLASS8 inbound_OBJs_src/$file | awk '{print $3}') =~ "255.0.0.0" ]]
                  then
                    SMATCH8ARR+=($file)
                fi
            done
...
#        echo "Source address $(echo $line | awk '{print $3}') Matched a host"
        SMATCHHA=($(grep -w $(echo $line | awk '{print $3}') inbound_OBJs_src/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
...
function destination_match
{
    ...
    DHOST=$(echo $line | awk '{print $5}')
        ...
        DMATCHSUB24="$(grep -w $CLASS24 inbound_OBJs/* | head -1)"
        DMATCHSUB24A=($(grep -w $CLASS24 inbound_OBJs/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        DMATCHSUB23="$(grep -w $CLASS23 inbound_OBJs/* | head -1)"
        DMATCHSUB23A=($(grep -w $CLASS23 inbound_OBJs/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        DMATCHSUB16="$(grep -w $CLASS16 inbound_OBJs/* | head -1)"
        DMATCHSUB16A=($(grep -w $CLASS16 inbound_OBJs/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        DMATCHSUB12="$(grep -w $CLASS12 inbound_OBJs/* | head -1)"
        DMATCHSUB12A=($(grep -w $CLASS12 inbound_OBJs/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        DMATCHSUB8="$(grep -w $CLASS8 inbound_OBJs/* | head -1)"
        DMATCHSUB8A=($(grep -w $CLASS8 inbound_OBJs/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
        ...
        if test -n ${DMATCHSUB24A[0]}
          then
            ...
            for file in ${DMATCHSUB24A[@]}
              do
                if [[ $(grep -w $CLASS24 inbound_OBJs/$file | awk '{print $3}') =~ "255.255.255.0" ]]
                  then
                    DMATCH24ARR+=($file)
                fi
            done
        ...
        if test -n ${DMATCHSUB23A[0]}
          then
            ...
            for file in ${DMATCHSUB23A[@]}
              do
                if [[ $(grep -w $CLASS23 inbound_OBJs/$file | awk '{print $3}') =~ "255.255.254.0" ]]
                  then
                    DMATCH23ARR+=($file)
                fi
            done
        ...
        if test -n ${DMATCHSUB16A[0]}
          then
            ...
            for file in ${DMATCHSUB16A[@]}
              do
                if [[ $(grep -w $CLASS16 inbound_OBJs/$file | awk '{print $3}') =~ "255.255.0.0" ]]
                  then
                    DMATCH16ARR+=($file)
                fi
            done
        ...
        if test -n ${DMATCHSUB12A[0]}
          then
            ...
            for file in ${DMATCHSUB12A[@]}
              do
                if [[ $(grep -w $CLASS12 inbound_OBJs/$file | awk '{print $3}') =~ "255.240.0.0" ]]
                  then
                    DMATCH12ARR+=($file)
                fi
            done
        ...
        if test -n ${DMATCHSUB8A[0]}
          then
            ...
            for file in ${DMATCHSUB8A[@]}
              do
                if [[ $(grep -w $CLASS8 inbound_OBJs/$file | awk '{print $3}') =~ "255.0.0.0" ]]
                  then
                    DMATCH8ARR+=($file)
                fi
            done
...
#        echo "destination address $(echo $line | awk '{print $5}') Matched a host"
        DMATCHHA=($(grep -w $(echo $line | awk '{print $5}') inbound_OBJs/* | awk -F"/" '{print $2}' | awk -F":" '{print $1}'))
...
while read -r line
  do
    ...
    DMATCH="$(grep -w $(echo $line | awk '{print $5}') inbound_OBJs/* | head -1 | awk '{print $1}' | awk -F"/" '{print $2}')"
    # The port number and TCP/UDP
    DMATCH2="$(echo $line | awk '{print $1,$6}')"
    DMATCH3="$(grep -w $(echo $line | awk '{print $5}') inbound_OBJs/* | head -1)" # | awk '{print $1}' | awk -F"/" '{print $2}')"
    grep -w $(echo $line | awk '{print $5}') inbound_OBJs/* | head -1 | awk '{print $1}' | awk -F"/" '{print $2}' >/dev/null || echo "$(echo $line | awk '{print $5}') HAS NO DMATCH (line 200)"
    #Debugging
#    echo "DMATCH   == $DMATCH"
...
#    echo "DMATCH3  == $DMATCH3"
    ...
    SMATCH="$(grep -w $(echo $line | awk '{print $3}') inbound_OBJs_src/* | head -1 | awk '{print $1}' | awk -F"/" '{print $2}')"
    SMATCH3="$(grep -w $(echo $line | awk '{print $3}') inbound_OBJs_src/* | head -1)" # | awk '{print $1}' | awk -F"/" '{print $2}')"
    grep -w $(echo $line | awk '{print $5}') inbound_OBJs/* | head -1 | awk '{print $1}' | awk -F"/" '{print $2}' >/dev/null || echo "$(echo $line | awk '{print $5}') HAS NO SMATCH (line 210)"
...
#           echo "destination is $destination"
            unset ACLMATCH
            unset NOMATCH
            ACLMATCH1=$(echo $line | grep "access-list outside_acl extended permit ip object-group $source")
...
#               echo "checking if destination host matches"
    ...
    if [ $ACLBREAK -eq 0 ]
      ...
      echo "Adding \"service $(echo $DMATCH2 | awk '{print $1}') destination eq $(echo $DMATCH2 | awk '{print $2}')\" to inbound_service_OBJs/$(echo $ACLMATCHD)-ServiceOBJ"
      echo "service $(echo $DMATCH2 | awk '{print $1}') destination eq $(echo $DMATCH2 | awk '{print $2}')" >> inbound_service_OBJs/$(echo $ACLMATCHD)-ServiceOBJ
    fi
     echo "================================================"
done < $1
